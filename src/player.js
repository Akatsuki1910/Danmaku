...
function onpointmain() {
  $("#pixiview").on("mousemove", (event) => {
    onpointmove(event);
  });
  $("#pixiview").on("mouseleave", () => {
    $("#pixiview").unbind("mouseup mousemove mouseleave");
  });
  $("#pixiview").on("mouseup", () => {
    $("#pixiview").unbind("mouseup mousemove mouseleave");
  });
...
function playermouse(playx, playy, min, max) {
  if (playx - min < 0) {
    ...
    move_flg = 1;
  } else if (playx + max > x * 2) {
    move_flg = 2;
  } else if (playy + max > y * 2) {
    move_flg = 3;
  }
}
